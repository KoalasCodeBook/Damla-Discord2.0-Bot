...
def film_imdb_sorgula(link,user): #movie send request
...
def film_veri_ekle(name,url,image,description,datePublished,genre,user,FilmTarzi): #add movie to db
    ...
    if len(temp_data) == 0:
        ...
        con.commit()  
        return  name + " adlı filmini senin adına veri tabanına ekledim"
    else:
      OldUser = temp_data[0][7]     
      if OldUser == user:
        return "Lan Yarrak bu filmi zateeen ekledin"
      else:
          return "Sanırım bu filmi senden önce " + OldUser + " keşfetmiş...Seni Aptal!" 
def film_veri_listele():  #print movie data
...
def nasilsin_veri_ekle(veri):
  con = sqlite3.connect(database_path)
  cursor = con.cursor()   
  cursor.execute('CREATE TABLE IF NOT EXISTS "nasilsin" ("id" TEXT,"veri" TEXT)') 
  cursor.execute("INSERT INTO nasilsin (veri) VALUES (?)",(veri,))  
  con.commit()
...
#-----User Arrangement-*-----------
def user_duzenle(user): #turn user ID from int to string
  id = ""
